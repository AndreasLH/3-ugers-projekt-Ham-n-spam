...
def split5050(x,y):
    """imputs text column as x and spam/ham (0/1) as y"""
    #create empty arrays and counters
    spam_index = np.array([])
    ham_index = np.array([])
    count_spam = 0
    count_ham = 0
    ...
    spam_text = x.drop(ham_index, axis = 0)
#pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.drop.html
    spam_class = y.drop(ham_index, axis = 0)

    spam_text.index = np.arange(0,count_spam)
    #https://www.geeksforgeeks.org/python-pandas-series-rename/
    ...
    train_spam_text = spam_text.loc[:numb_class-1]
